...
TEST(NAME, insert_new_alloc)
{
    ...
    force_malloc_fail_on();
    ASSERT_THAT(ordered_vector_insert(vec, 0, &a), Eq(0));
    force_malloc_fail_off();
    ASSERT_THAT(vec->count, Eq(0));
    ASSERT_THAT(vec->capacity, Eq(0));
    ASSERT_THAT(vec->data, IsNull());

    ASSERT_THAT(ordered_vector_insert(vec, 0, &a), Ne(0));
    ASSERT_THAT((int*)ordered_vector_back(vec), Pointee(a));
    ASSERT_THAT(vec->count, Eq(1));
...
TEST(NAME, insert_realloc)
{
    ...
    void* old_data_ptr = vec->data;
    force_malloc_fail_on();
    ASSERT_THAT(ordered_vector_insert(vec, 1, &a), Eq(0));
    force_malloc_fail_off();
    ASSERT_THAT(vec->count, Eq(vec->capacity));
    ASSERT_THAT(vec->data, Eq(old_data_ptr)); /* make sure data is preserved */

    ASSERT_THAT(ordered_vector_insert(vec, 1, &a), Ne(0));
    ASSERT_THAT(vec->count, Lt(vec->capacity));
    ASSERT_THAT(vec->data, Ne(old_data_ptr));
...
TEST(NAME, insert_emplace_new_alloc)
{
    ...
    force_malloc_fail_on();
    ASSERT_THAT(ordered_vector_insert_emplace(vec, 0), IsNull());
    force_malloc_fail_off();
    ASSERT_THAT(vec->count, Eq(0));
    ASSERT_THAT(vec->capacity, Eq(0));
    ASSERT_THAT(vec->data, IsNull());
...
TEST(NAME, insert_emplace_realloc)
{
    ...
    void* old_data_ptr = vec->data;
    force_malloc_fail_on();
    ASSERT_THAT(ordered_vector_insert_emplace(vec, 1), IsNull());
    force_malloc_fail_off();
    ASSERT_THAT(vec->count, Eq(vec->capacity));
    ASSERT_THAT(vec->data, Eq(old_data_ptr)); /* make sure data is preserved */

    ASSERT_THAT(ordered_vector_insert_emplace(vec, 1), NotNull());
    ASSERT_THAT(vec->count, Lt(vec->capacity));
    ASSERT_THAT(vec->data, Ne(old_data_ptr));
